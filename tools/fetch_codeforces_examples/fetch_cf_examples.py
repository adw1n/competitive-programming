...
class Problem:
    ...
    def __init__(self,problem_link: str, examples: typing.List[str]):
...
class Codeforces:
    ...
    def get_problem(link: str) -> Problem:
        ...
        inputs = []  # type: typing.List[Example]
        outputs = []  # type: typing.List[Example]
    ...
    def get_problems(contest_link: str) -> typing.List[Problem]:
        ...
        assert contest.status_code == 200, "Could not fetch contest page: %s status code: %s"%(contest_link,contest.status_code)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--set-username', help='your username - used to deduce the contest you are participating in (when not using the --link flag),'
                                               ' when there are two concurrent contests for div 1 and div 2')
    parser.add_argument('--set-contest-dir', help='set contest directory - examples are saved in $CONTEST_DIR/$PROBLEM_NAME/in(ID).txt')
